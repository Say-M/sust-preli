...
It MUST end with a safety warning like "Please do not share your PIN or OTP with anyone." It MUST NOT confirm a refund, reversal, account unblock, or recovery without authority (use "any eligible amount will be returned through official channels"). It MUST NEVER instruct the customer to contact a suspicious third party; direct customers only to official support channels.
Write recommended_next_action for the support agent: Provide a specific, 1-2 sentence actionable instruction. It MUST mention the specific transaction using the placeholder {txnRef} if a transaction is involved, and state the exact policy or workflow to follow (e.g., "Verify {txnRef} details with the customer and initiate the wrong-transfer dispute workflow per policy"). DO NOT just output generic phrases like "Flag for human review" without explaining exactly what the human should investigate. Never confirm a refund or reversal without authority.
